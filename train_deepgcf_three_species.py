...
def setTrainingHyperParameters():
    ...
    # CL: added one more list element for third species
    num_layers = [a,a,a,b] # human-specific, pig-specific, sp3 specific, and final
    
    # CL: added two more list elements for third species
    num_neurons = [0,0,0,0,0,0,0,0]

    for i in range(a):
        num_neurons[i] = randomNumNeurons()
        num_neurons[i+2] = randomNumNeurons()
        # CL: added this line for third species
        num_neurons[i+4] = randomNumNeurons()
    for j in range(b):
        # CL: changed to j+6 instead of j+4
        num_neurons[j+6]= randomNumNeurons()
...
def train(human_data_filename,pig_data_filename,shuffled_pig_data_filename,
            # CL: added extra parameters for third species
            sp3_data_filename, shuffled_sp3_data_filename,
    ...
    with open(human_data_filename,'r') as positive_human_file,\
            ...
            open(sp3_data_filename, 'r') as positive_sp3_file, \
            open(shuffled_sp3_data_filename,'r') as negative_sp3_file:
        next(positive_human_file)
        next(positive_pig_file)
        next(negative_human_file)
        next(negative_pig_file)
        # CL: added for third species
        next(positive_sp3_file)
        next(negative_sp3_file)
        ...
        for i in range(num_batch):
            ...
            weights = torch.FloatTensor(weights).to(device) ### CL: added ".to(device)" to activate GPU, can remove
            criterion = nn.BCELoss(weight=weights) # binary cross entropy loss

            # Read data for the current training batch
            # changed by CL to add third species
            ftr = [positive_human_file,positive_pig_file, positive_sp3_file, negative_human_file,negative_pig_file, negative_sp3_file]
            inputs,labels = readBatch(files_to_read=ftr,
                                      batch_size=current_batch_size,
                                      num_features=num_features)
                                      
            ### added by CL to activate GPU on Google Colab, can remove -------------------
    ...
    return net,torch.Tensor.cpu(running_loss).numpy() #changed by CL to convert gpu tensor to numpy, can just change line to "return net, running_loss" if explicit call to use gpu not required

# Evaluate the current model with held-out validation data
def eval(human_data_filename,pig_data_filename,shuffled_pig_data_filename,sp3_data_filename,shuffled_sp3_data_filename,
num_batch,batch_size,positive_data_size,num_features,net):
    ...
    with open(human_data_filename,'r') as positive_human_file,\
        ...
        for i in range(num_batch):
            ...
            # CL: added sp3 files
            inputs,_ = readBatch(files_to_read=[positive_human_file,positive_pig_file,positive_sp3_file,
            negative_human_file,negative_pig_file, negative_sp3_file],
                                 batch_size=current_batch_size,
                                 num_features=num_features)
                                 
            ### added by CL to activate GPU on Google Colab, can remove first line and change second line to "preds = net(inputs)" 
            inputs = inputs.to(device)
            preds = net(inputs).to(device)
            ### ---------------------------------------------

            # Make predictions using the current model
            score_y[i*batch_size:i*batch_size+current_batch_size] = torch.Tensor.cpu(preds).data.numpy() #changed by CL to convert gpu tensor to numpy, if not needed can change right hand side to just "= preds.data.numpy()"
...
def findLineOffsets(total_positive_training_data_size,
                    ...
                    # CL: added for third species
                    sp3_training_data_filename, shuffled_sp3_training_data_filename):
    
    #CL: changed from 3 to 5
    offsets = np.zeros((5,total_positive_training_data_size),dtype=int) # array of offsets
    ...
    # CL added        
    offset = 0
    with open(sp3_training_data_filename, 'r') as f:
        for i in range(total_positive_training_data_size):
            line = f.readline()
            offsets[3,i] = offset
            offset += len(line)
    ...
    # CL added  
    offset = 0
    with open(shuffled_sp3_training_data_filename, 'r') as f:
        for i in range(total_positive_training_data_size):
            line = f.readline()
            offsets[4,i] = offset
            offset += len(line)
...
def main():
    ...
    ### added by CL to activate GPU, can remove -------------------
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ### ---------------------------------------------
    
    ### Input arguments start here ###
    parser = argparse.ArgumentParser(prog='python src/train_deepgcf_three_species.py', description='Train a neural network')
    ...
    # added by CL ---
    g1.add_argument('-G', '--sp3-training-data-filename', required=True,
                    help='path to sp3 positive training data file', type=str)
    g1.add_argument('-H', '--shuffled-sp3-training-data-filename', required=True,
                    help='path to sp3 shuffled/negative training data file', type=str)
    # ---------------
    ...
    # added by CL ---
    g1_2.add_argument('-I', '--sp3-validation-data-filename', required=True,
                      help='path to sp3 positive validation data file', type=str)
    g1_2.add_argument('-J', '--shuffled-sp3-validation-data-filename', required=True,
                      help='path to sp3 shuffled/negative validation data file', type=str)
    # ---------------
    ...
    # added by CL - set defaults to match pig
    g4.add_argument('-spf', '--num-sp3-features',
    ...
    # added by CL - set defaults to same as pig
    g2.add_argument('-nnsp1', '--num-neuron-sp3-1',
                    help='number of neurons in the first hidden layer in the sp3-specific sub-network (default: 128)', type=int,
                    default=128)
    g2.add_argument('-nnsp2', '--num-neuron-sp3-2',
                    help='number of neurons in the second hidden layer in the sp3-specific sub-network (default: 0)', type=int,
                    default=0)
    # -----------------------------------------
    ...
    if args.random_search:
        ...
        # CL: added one extra num_layers 1 for species 3
        num_layers = [args.num_layers_1,args.num_layers_1,args.num_layers_1,args.num_layers_2]
        # CL: added parameters for species 3
        num_neurons = [args.num_neuron_human_1,args.num_neuron_human_2,args.num_neuron_pig_1,args.num_neuron_pig_2,
        args.num_neuron_sp3_1,args.num_neuron_sp3_2,args.num_neuron_1,args.num_neuron_2]
    ...
    hyperparameter_names = ['Seed',
                            ...
                            # added by CL
                            'Number of layers in sp3-specific network',
                            ...
                            # added by CL
                            'Number of neurons in the 1st layer of sp3-specific network',
                            'Number of neurons in the 2nd layer of sp3-specific network',
    ...
    if args.total_positive_training_data_size>args.positive_training_data_size:
        ...
        offsets = findLineOffsets(args.total_positive_training_data_size,
                                  ...
                                  args.sp3_training_data_filename,args.shuffled_sp3_training_data_filename)

        # Select lines from the training data to use
        print ('Selecting lines to read for training data...')
        # CL: changed from 4 to 6 for extra third species
        lines_to_read = np.zeros((6,args.positive_training_data_size),dtype=int)
        ...
        # CL note: this is the orthologous data
        ...
        # CL: added this for third species
        lines_to_read[4,:] = offsets[4,positive_samples_to_read] # Positive sp3 training data
        
        # CL note: this is the non-orthologous data
        ...
        # CL: added this for third species
        lines_to_read[5,:] = sorted(random.sample(list(offsets[5,:]),k=args.positive_training_data_size)) # Negative sp3 training data
    else:
        lines_to_read = -1

    # Build neural network
    net = PseudoSiameseNet(args.num_human_features, args.num_pig_features, args.num_sp3_features, num_layers, num_neurons, dropout_rate)
    ...
    for epoch in range(args.num_epoch): # loop over the training data multiple times
        ...
        net,running_loss = train(args.human_training_data_filename,
                                 ...
                                 # added by CL for third species
                                 args.sp3_training_data_filename,
                                 args.shuffled_sp3_training_data_filename,
                                 ...
                                 # CL added element for third species
        ...
        validation_result = eval(args.human_validation_data_filename,
                                 ...
                                 # CL added for third species
                                 args.sp3_validation_data_filename,
                                 args.shuffled_sp3_validation_data_filename,
